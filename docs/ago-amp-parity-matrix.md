...
| Intelligent board orchestration | Ago differentiation | one repository and objective become a context-informed hierarchical work graph that is assigned, monitored, retried, and verified by the board agent | partial | durable threads, routing, parent/child relations, mailbox, receipts, and verification evidence | add repository context packages; make the board the coordination authority; give workers bounded context; prove recursive planning, dependency-aware scheduling, reassignment, and project-level completion; Phase 3 |
