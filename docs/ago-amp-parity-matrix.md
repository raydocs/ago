...
Classification describes the current repository before Ago's new durable thread
...
| Automatic capability routing | Ago differentiation | users never configure models; stage routes use evaluated best capability | partial | catalog/router/routeeval/lane health/usage | integrate with Ago-owned runtime and prove against single-model baseline; Phase 7 |

## Baseline conclusion

The repository already contains meaningful routing, observability, transcript,
retrieval, and archive assets. It does **not** yet contain the substrate Amp's
rebuilt era depends on: an Ago-owned durable thread state machine, mailbox,
headless daemon, executor contract, and reconnectable client protocol. Phase 1
must build that substrate rather than extending transcript observation into an
implicit runtime.
